...
`.bundle` and a Windows `.dll`. MIT. Public at
`github.com/stoatworks-labs/photofinish` since 2026-09-23, released at v0.1.0
on 2026-09-23. User guide: `docs/USER-GUIDE.md` (the only copy anyone edits),
rendered to `docs/USER-GUIDE.pdf` and to
https://stoatworks-labs.com/software/photofinish/guide/ by the website's
`build_guides.py`.
...
- **The GPU-less rasteriser is no longer a gap (2026-09-23).** `ci.yml` ran on GitHub's macOS runner, which has no GPU, so
  the harness ran on Apple's software renderer. All twelve ctest checks
  (`--schedule`, `--static`, `--ring`, `--clock`, `--interp`, `--width`,
  `--matched`, `--reverse`, `--sync`, `--slit`, `--resize`, `--negative`) and
  the control sweep (`tools/sweep.py --size 160x90 --frames 24`) passed there.
  The argument above — that none of these checks *can* depend on the
  rasteriser — has held on a second one.
...
- **On GitHub (2026-09-23):** every ctest check and the control sweep passed on
  the GPU-less macOS runner (Apple's software renderer — see the audit above),
  and the Windows x64 DLL compiled with MSVC, in `ci.yml` and `release.yml`.
- **18 parameters** (`pftest --list`): the 13 controls and a five-entry About
  block (About, User guide, Project page, Source on GitHub, Support the work).
  `source/StoatworksAbout.h` is generated by stoatworks-backend's
  `sync-about.py` and `ATTRIBUTIONS.md` by `sync-attributions.py`; the project
  is registered in the website's projects.json (beta, with a guide) and in the
  backend's sync-about TARGETS, names.json, visibility.json and derived.json.
- **Windows, in Resolume Arena 7.27.1** (win-lab, llvmpipe, 2026-09-23): a CI build loads, registers as `SW Photofinish` / `PF01` / effect, matches all 19 host parameters and renders — 8 of 9 gate checks. The time controls (Time Per Column, Interpolate, Fill, Background, Freeze) read dead on the gate's still picture, which a strip camera turns into identical streaks at any setting; the sweep proves them live on a moving card.
...
- **Never loaded into Resolume on macOS.** Everything here was compiled, rendered and
  measured offline against the real plugin class in a headless CGL context,
  plus one load in the fleet's own `oxbow` host. Nothing has driven Arena on macOS. How
...
- **Never run on an Intel Mac.** The build is universal and `lipo` says so, but
  only the arm64 slice has ever executed. The render cost above is macOS
  (Apple Silicon) only.
