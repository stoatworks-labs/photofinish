...
- Every check in one process: `./build/pftest --schedule --static --ring --clock
  --interp --width --matched --reverse --sync --slit --resize --negative`
...
| `--slit` | the slit reads the column it says it does — the only check that measures **space** rather than time |
...
- 18 parameters (`./build/pftest --list`): 13 controls, then the five-entry
  About block (About, User guide, Project page, Source on GitHub, Support the
  work). `source/StoatworksAbout.h` is GENERATED by stoatworks-backend's
  ...
  https://stoatworks-labs.com/software/photofinish/guide/ are rendered from it
...
## Released
- Public at github.com/stoatworks-labs/photofinish (created 2026-09-23),
  released at v0.1.0 on 2026-09-23. Registered on the website (projects.json,
  beta, with a guide) and in stoatworks-backend (sync-about TARGETS,
  names.json, visibility.json, derived.json).
- CI (`ci.yml`) has run on GitHub's GPU-less macOS runner, i.e. Apple's
  software renderer: every ctest check (`--schedule` through `--negative`) and
  `tools/sweep.py --size 160x90 --frames 24` passed there. The Windows x64 DLL
  has been compiled with MSVC by `ci.yml` and `release.yml`. The Windows
  Arena result is recorded in README Status and AGENTS.md.
